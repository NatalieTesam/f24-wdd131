...
function renderTasks(tasks) {
  ...
  // takes the list of strings and smashes them together into one big string.
...
function newTask() {
  ...
  const task = {detail: todo.value, completed: false};
  tasks.push(task);
...
function manageTasks(event) {
  ...
  // because we added 'data-action="delete"' to each icon in a task we can access 
  // a dataset property on our target (e.target.dataset.action)
...
// We need to attach listeners to the submit button and the list. Listen for
// a click, call the 'newTask' function on submit and call the 'manageTasks' 
